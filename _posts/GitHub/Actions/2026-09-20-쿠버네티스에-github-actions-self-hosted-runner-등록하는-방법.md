...
3. Docker Hub 이미지를 캐시하는 레지스트리를 `arc-systems` 네임스페이스에 설치
4. 토큰을 `arc-runners-[저장소 또는 조직 이름]` 네임스페이스의 Secret으로 저장
5. 아래 표의 러너 세 종류를 설치하고, GitHub에 등록되어 listener 파드가 실행될 때까지 대기
...
Docker Hub 이미지는 클러스터 안의 캐시를 거쳐 받으며, `ghcr.io` 같은 다른 레지스트리의 이미지는 캐시되지 않습니다. 보존 기간(기본 30일)과 최대 용량(기본 20Gi)은 스크립트 맨 위의 `CACHE_TTL`과 `CACHE_SIZE`에서 바꿉니다.

- **확인:** 마지막에 세 러너의 이름과 자원이 출력되고, GitHub 저장소(조직 계정은 조직)의 **Settings** > **Actions** > **Runners**에 러너 세 개 표시, `kubectl get pods -n arc-systems`에 `docker-hub-cache`로 시작하는 파드가 `Running`
